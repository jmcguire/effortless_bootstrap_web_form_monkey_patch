...
import effortless_bootstrap_web_form_monkey_patch as ebwfmp
...
class TestDropInClass(unittest.TestCase):
  ...
  def test_dropdown_render(self):
    ...
    result2 = remove_optional_whitespace(ebwfmp.Dropdown(name='foo', args=[('a', 'aa'), ('b', 'bb'), ('c', 'cc')], value='b').render())
  ...
  def test_radio_render(self):
    result1 = remove_optional_whitespace(ebwfmp.Radio(name='foo', args=['a', 'b'], value='b').render())
    ...
    result2 = remove_optional_whitespace(ebwfmp.Radio(name='foo', args=[('a', 'aa'), ('b', 'bb'), ('c', 'cc')], value='b').render())
  ...
  def test_button_render(self):
    result = ebwfmp.Button(name='foo').render()
    correct = '<button class="btn btn-primary" type="submit" id="foo" name="foo">foo</button>'
    self.assertEqual(result, correct)

  def test_form_render(self):
    result = remove_optional_whitespace(ebwfmp.Form(ebwfmp.Textbox("x")).render())
  ...
  def test_form_with_error_render(self):
    input_field = ebwfmp.Textbox("x")
    input_field.note = "ERROR"
    result = remove_optional_whitespace(ebwfmp.Form(input_field).render())
